...
async def contact_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        async with async_session() as session:
            ...
            import os
            frontend_url = os.getenv("FRONTEND_URL", "https://kreditscore4-frontend-production.up.railway.app")
            auth_link = f"{frontend_url}/?auth_token={auth_token}"
            
            success_message = (
                "✅ Отлично! Ваши данные получены.\n\n"
                "📱 Номер телефона сохранен\n"
                "💰 Данные займа сохранены\n\n"
                "🔗 **Нажмите на ссылку ниже для завершения авторизации:**\n"
                f"{auth_link}\n\n"
                "После перехода по ссылке вы можете закрыть этот чат."
            )
